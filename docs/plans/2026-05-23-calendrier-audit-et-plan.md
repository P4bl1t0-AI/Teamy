...
**Date de mise à jour** : 2026-05-23 (session terminée)  
**Projet** : Teamy  
**Repo local** : `/home/p4bl1/projects/teamy/`  
**Supabase** : `https://pvlcmthyhwssllhlibwt.supabase.co`  
**Branche** : `main` (commits `873d00c`, `91d8ee7`)  
...
- **NOUVEAUX** (calendrier) :
  - `CalendarGrid.tsx` — grille mensuelle avec nav
  - `DayCell.tsx` — cellule jour avec badges membres
  - `DayEditModal.tsx` — édition du statut d'un membre sur un jour
...
- **NOUVEAUX** (`src/app/calendar-actions.ts`) :
...
src/components/members/MemberList.tsx       (import @/types)
...
- Édition des jours par défaut d'un membre existant (pas seulement à la création)
- Filtrage des membres affichés sur le calendrier
- Export PDF/iCal du calendrier
- Vue semaine/vue liste alternative
